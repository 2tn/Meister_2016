...
#define	CCLK		48000000UL	/* cclk frequency [Hz] */
#define PCLK_SSP	48000000UL	/* PCLK frequency for SSP [Hz] */
#define SCLK_FAST	8000000UL	/* SCLK frequency under normal operation [Hz] */
#define	SCLK_SLOW	400000UL	/* SCLK frequency under initialization [Hz] */

#define	MMC_CD		!((LPC_GPIO->PIN[0] & (1 << 14)) >> 14)	/* Card detect (yes:true, no:false, default:true) */
...
#define	CS_LOW()	{LPC_GPIO->CLR[0] = (1 << 16);}	/* Set P0.16 low */
#define	CS_HIGH()	{LPC_GPIO->SET[0] = (1 << 16);}	/* Set P0.16 high */
#define PCSSPx		PCSSP0
#define	PCLKSSPx	PCLK_SSP0
#if 0
#define ATTACH_SSP() {\
		LPC_IOCON->PIO0_6 &= ~0x7;\
		LPC_IOCON->PIO0_6 |= 0x2; 	/* SCK0 */\
		LPC_IOCON->PIO0_9 &= ~0x7;\
		LPC_IOCON->PIO0_8 |= 0x1;	/* MISO0 */\
		LPC_IOCON->PIO0_9 &= ~0x7;\
		LPC_IOCON->PIO0_9 |= 0x1;	/* MOSI0 */\
		LPC_GPIO->DIR[0] = (1 << 16);\
		LPC_IOCON->PIO0_16 &= ~0x7;\
		LPC_IOCON->PIO0_16 = 0x0; 	/* CS# (P0.16) */\
		}
#else
#define ATTACH_SSP() {\
		LPC_IOCON->SWCLK_PIO0_10 &= ~0x7;\
		LPC_IOCON->SWCLK_PIO0_10 |= 0x2; 	/* SCK0 */\
		LPC_IOCON->PIO0_9 &= ~0x7;\
		LPC_IOCON->PIO0_8 |= 0x1;	/* MISO0 */\
		LPC_IOCON->PIO0_9 &= ~0x7;\
		LPC_IOCON->PIO0_9 |= 0x1;	/* MOSI0 */\
...
#define	SSPxCR1		LPC_SSP1->CR1
#define	SSPxCPSR	LPC_SSP1->CPSR
#define	CS_LOW()	{LPC_GPIO->CLR[1] = (1 << 19);}	/* Set P1.19 low */
#define	CS_HIGH()	{LPC_GPIO->SET[1] = (1 << 19);}	/* Set P1.19 high */
#define RST_N	(1 << 2)
#define	PCLKSSPx	(1 << 18)
#define SSPxCLKDIV	LPC_SYSCON->SSP1CLKDIV
#define ATTACH_SSP() {\
		...
		LPC_IOCON->PIO1_19 = 0x0; 	/* CS# (P1.19) */\
		LPC_GPIO->DIR[1] |= (1 << 19);\
		LPC_IOCON->TRST_PIO0_14 &= ~0x7;\
		LPC_IOCON->TRST_PIO0_14 |= 0x1; 	/* CD# (P0.14) */\
		LPC_GPIO->DIR[0] |= (0 << 14);\
		}
#endif
...
{
	LPC_SYSCON->PRESETCTRL |= RST_N; /* Enable SSP module */
	LPC_SYSCON->SYSAHBCLKCTRL |= PCLKSSPx; /* Select PCLK frequency for SSP */
	SSPxCLKDIV = CCLK / PCLK_SSP;

	SSPxCR0 = 0x0707; /* Set mode: SPI mode 0, 8-bit */
