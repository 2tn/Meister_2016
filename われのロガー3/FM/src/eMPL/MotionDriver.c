...
void quat2euler(QUAT_DATA *quat) {
	...
	if (calibration_mode & (1 << 0)) {
		if (!(calibration_mode & (1 << 3))) {
			calibration_count = 0;
			calibration_data[0] = 0;
			calibration_data[1] = 0;
			calibration_mode |= (1 << 3);
		}
		calibration_data[0] = (calibration_data[0] * calibration_count
				+ quat->euler[0]) / (calibration_count + 1);
		calibration_data[1] = (calibration_data[1] * calibration_count
				+ quat->euler[1]) / (calibration_count + 1);
		calibration_count++;
	}

	quat->euler[0] = (long) (values[0] * 65536.f) - calibration_data[0];
	quat->euler[1] = (long) (values[1] * 65536.f) - calibration_data[1];
	quat->euler[2] = (long) (values[2] * 65536.f);
}
