...
FATFS** fs /* Pointer to pointer to the owner file system object to return */
	...
	if (!dfp || !obj->fs || !obj->fs->fs_type || obj->fs->id != obj->id
...
DWORD get_fattime(void) {
	DWORD time = (nowdata.gps.year - 1980 << 25) | (nowdata.gps.month << 21)
			| (nowdata.gps.day << 16) | (nowdata.gps.hour << 11)
			| (nowdata.gps.min << 5) | (nowdata.gps.sec << 0);
	return time;
}
